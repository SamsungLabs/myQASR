...
def quantize_matmul_input(
    input_v, interval, qmax, n_G, n_V, n_H, crb_groups, crb_rows, crb_cols
):
    """
    quantize input matrix of matmul operation, with respect to sublayerwise padding settings
    """
    pad_groups = crb_groups * n_G - input_v.shape[1]
    pad_rows = crb_rows * n_V - input_v.shape[2]
    pad_cols = crb_cols * n_H - input_v.shape[3]

    x = F.pad(input_v, [0, pad_cols, 0, pad_rows, 0, pad_groups])
...
def quantize_int_activation(module, val_input):
    """
    Quantize current inputs into uint8 and store them as an attribute of the module.

    The function is a pre-forward hook that
    need to be manually added to the calibrated model.
    You need to manipulate the cached data before
    feeding another batch of pictures.
    Currently only support int8. (For twin quantization, we use uint8)

    For twin quantization:
    - For softmax, the MSB being 1 means using large interval,
    while MSB being 0 means using small interval.
    - For post-GELU, the MSB serves as sign bit.
    We use 1 for positive values and 0 for negative values.
    """
...
def get_model_int_weight(wrapped_modules):
    ...
    for name, m in wrapped_modules.items():
        try:
            ...
            # Log the exception or simply pass if you don't need to handle it.
            # This is a placeholder for logging. Replace it with your logging library or print statement.
            print(f"Exception occurred while quantizing weights for module {name}: {e}")
