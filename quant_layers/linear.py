...
class MinMaxQuantLinear(nn.Linear):
    """quantize to min and max values"""
    ...
    def quant_forward_basic(self, x):
        ...
        out = F.linear(q_x, q_w, self.bias)
        # out = F.conv1d(q_x, q_w, self.bias, self.stride,
    ...
    def quant_forward(self, x):
        """quantize during forward operation"""
        assert (
    ...
    def _bias_correction_quant_forward(self, x):
        """correct bias during quantization"""
    ...
    def calibration_step1(self, x):
        """perform first callibration step to store raw values"""
...
class PTQSLQuantLinear(MinMaxQuantLinear):
    ...
    def _initialize_intervals(self, x=0):
...
class PTQSLBatchingQuantLinear(PTQSLQuantLinear):
    """Quantization with batching for efficient inference"""
    ...
    def _search_best_w_interval(self, weight_interval_candidates):
        """find best weight intervals"""
        tmp_w_interval = self.w_interval.unsqueeze(0)  # shape: 1,n_V,1,n_H,1
        for h in range(self.n_H):
            batch_similarities = []  # similarities, need to concatenate
            # and calculate sum (equivalent to mean with argmax)
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                b_ed = min(self.calib_size, b_st + self.calib_batch_size)
    ...
    def _search_best_a_interval(self, input_interval_candidates):
        """find best activation intervals"""
        tmp_a_interval = self.a_interval.unsqueeze(-1)  # shape: n_a,1,1
        for a in range(self.n_a):
            batch_similarities = []  # similarities, need to concatenate and
            # calculate sum (equivalent to mean with argmax)
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                b_ed = min(self.calib_size, b_st + self.calib_batch_size)
                x = self.raw_input[b_st:b_ed].cuda()
                raw_out_expanded = (
...
class PostGeluPTQSLBatchingQuantLinear(PTQSLBatchingQuantLinear):
    ...
    use a_interval for positive activation
    quantization and a_neg_interval for negative activation quantization
    """
